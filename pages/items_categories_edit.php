<?PHP
	...
	if (!empty($_GET['id'])) {
		...
		$sql = "SELECT * FROM `category` WHERE `cid`='$id_id'";
			...
			if (!preg_match('!^[\w -]*$!', $_POST['name'])) { 
				...
				if (is_numeric($id_id)) {
					...
					if (mysqli_num_rows($result) < 1) { $panel_type = 'panel-danger'; $panel_notice = "ERROR: wrong ID."; }
						...
						$sql = "UPDATE `category` SET `name`='$name' WHERE `cid`='$id_id'";
						$result = mysqli_query($link, $sql);
						if ($result) {
							$panel_type = 'panel-success';
							$panel_notice = "Category has been Changed. <a href=\"items_categories.php\" title=\"Return\" alt=\"Return\">Return to categories</a>";
						}
						else {
							$panel_type = 'panel-danger';
				...
				<div class="col-lg-12">
					...
					<div class="panel-body">
						<form role="form" method="post">
							...
							<div class="form-group">
								<input class="form-control" type="hidden" placeholder="" name="id" value="<?PHP if (!empty($_REQUEST['cid'])) { print $_REQUEST['cid']; } ?>" disabled>
							</div>
